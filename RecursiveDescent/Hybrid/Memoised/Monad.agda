...
-- Note: Some of the code below has been replaced by postulates since
-- type checking it requires too much memory.
...
module PM where
  ...
  adjustBound hyp (pm m) =
    pm \xs -> let le = \ys -> trans (bounded ys) (hyp (position xs)) in
