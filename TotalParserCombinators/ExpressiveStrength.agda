...
module Monadic where
  ...
  corollary : ∀ {Tok R} (f : List Tok → List R) {x s} →
              x ∈ parseComplete (grammar f) s ⇿ x ∈ f s
  corollary f {x} {s} =
    (x ∈ parseComplete (grammar f) s)  ⇿⟨ sym Backend.correct ⟩
...
module Applicative where
  ...
  corollary : ∀ {R} (f : List Bool → List R) {x s} →
              x ∈ parseComplete (grammar f) s ⇿ x ∈ f s
  corollary f {x} {s} =
    (x ∈ parseComplete (grammar f) s)  ⇿⟨ sym Backend.correct ⟩
    x ∈ grammar f · s                  ⇿⟨ grammar-correct f ⟩
