...
mutual
  ...
  ∞L⟨_⟩Parser : {R₂ : Set} →
                List R₂ → Set → (R₁ : Set) → List R₁ → Set₁
  ∞L⟨ []    ⟩Parser Tok R₁ xs = ∞ (Parser Tok R₁ xs)
  ∞L⟨ _ ∷ _ ⟩Parser Tok R₁ xs =    Parser Tok R₁ xs

  ∞M⟨_⟩Parser : {R₂ : Set} →
                Maybe R₂ → Set → (R₁ : Set) → List R₁ → Set₁
  ∞M⟨ nothing ⟩Parser Tok R₁ xs = ∞ (Parser Tok R₁ xs)
  ∞M⟨ just _  ⟩Parser Tok R₁ xs =    Parser Tok R₁ xs

  ∞⟨_⟩Parser : ∀ {t} {R₂ : Set} →
               El t R₂ → Set → (R₁ : Set) → List R₁ → Set₁
  ∞⟨_⟩Parser {list}  = ∞L⟨_⟩Parser
  ∞⟨_⟩Parser {maybe} = ∞M⟨_⟩Parser
...
⟦ _⊛_ {fs = []   } {xs = []   }    p₁ p₂ ⟧ = ⟪ ♯ ⟦ ♭ p₁ ⟧ ⟫ ⊛          ⟪ ♯ ⟦ ♭  p₂    ⟧ ⟫
⟦ _>>=_ {xs = _ ∷ _} {f = just _}  p₁ p₂ ⟧ = ⟨   ⟦   p₁ ⟧ ⟩ >>=  λ x → ⟨   ⟦    p₂ x  ⟧ ⟩
⟦ _>>=_ {xs = _ ∷ _} {f = nothing} p₁ p₂ ⟧ = ⟪ ♯ ⟦ ♭ p₁ ⟧ ⟫ >>=  λ x → ⟨   ⟦    p₂ x  ⟧ ⟩
⟦ _>>=_ {xs = []   } {f = just _}  p₁ p₂ ⟧ = ⟨   ⟦   p₁ ⟧ ⟩ >>=  λ x → ⟪ ♯ ⟦ ♭ (p₂ x) ⟧ ⟫
⟦ _>>=_ {xs = []   } {f = nothing} p₁ p₂ ⟧ = ⟪ ♯ ⟦ ♭ p₁ ⟧ ⟫ >>=  λ x → ⟪ ♯ ⟦ ♭ (p₂ x) ⟧ ⟫
...
⟦⟦ p ⟧⁻¹⟧ = record
  ...
  mutual
    ...
    from (_⊛_ {p₁ = ⟪ _ ⟫} {p₂ = ⟨ _ ⟩} f∈p₁ x∈p₂)              = from f∈p₁ ⊛ from x∈p₂
    from (_⊛_ {p₁ = ⟪ _ ⟫} {p₂ = ⟪ _ ⟫} f∈p₁ x∈p₂)              = from f∈p₁ ⊛ from x∈p₂
    from (_>>=_ {xs = _ ∷ _} {p₁ = ⟨ _ ⟩} {p₂ = p₂} x∈p₁ y∈p₂x) =   from x∈p₁ ⟨⟨ ⟨_⟩>>=_ {p₂ = λ x → ⟨ ⟦ ⟦♭ p₂ x ⟧⁻¹ ⟧ ⟩} ⟩⟩
                                                                                   from♭ (p₂ _) y∈p₂x
    from (_>>=_ {xs = _ ∷ _} {p₁ = ⟪ _ ⟫} {p₂ = p₂} x∈p₁ y∈p₂x) = ⟪ from x∈p₁ ⟫>>= from♭ (p₂ _) y∈p₂x
    from (_>>=_ {xs = []}    {p₁ = ⟨ _ ⟩}           x∈p₁ y∈p₂x) = ⟨ from x∈p₁ ⟩>>= from         y∈p₂x
    from (_>>=_ {xs = []}    {p₁ = ⟪ _ ⟫}           x∈p₁ y∈p₂x) = ⟪ from x∈p₁ ⟫>>= from         y∈p₂x
  ...
  mutual
    ...
    to∘from (_⊛_ {p₁ = ⟪ _ ⟫} {p₂ = ⟨ _ ⟩} f∈p₁ x∈p₂)              = Eq.cong₂ _⊛_ (to∘from f∈p₁) (to∘from x∈p₂)
    to∘from (_⊛_ {p₁ = ⟪ _ ⟫} {p₂ = ⟪ _ ⟫} f∈p₁ x∈p₂)              = Eq.cong₂ _⊛_ (to∘from f∈p₁) (to∘from x∈p₂)
    to∘from (_>>=_ {xs = _ ∷ _} {p₁ = ⟨ _ ⟩} {p₂ = p₂} x∈p₁ y∈p₂x) = Eq.cong₂ (⟨_⟩>>=_ {p₂ = p₂})
                                                                                       (to∘from x∈p₁) (to♭∘from♭ (p₂ _) y∈p₂x)
    to∘from (_>>=_ {xs = _ ∷ _} {p₁ = ⟪ _ ⟫} {p₂ = p₂} x∈p₁ y∈p₂x) = Eq.cong₂  ⟪_⟫>>=_ (to∘from x∈p₁) (to♭∘from♭ (p₂ _) y∈p₂x)
    to∘from (_>>=_ {xs = []}    {p₁ = ⟨ _ ⟩}           x∈p₁ y∈p₂x) = Eq.cong₂  ⟨_⟩>>=_ (to∘from x∈p₁) (to∘from          y∈p₂x)
    to∘from (_>>=_ {xs = []}    {p₁ = ⟪ _ ⟫}           x∈p₁ y∈p₂x) = Eq.cong₂  ⟪_⟫>>=_ (to∘from x∈p₁) (to∘from          y∈p₂x)
